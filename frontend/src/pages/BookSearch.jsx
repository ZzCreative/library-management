...
function BookSearch() {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      {selectedBook && (
        <div onClick={closeDetails} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '12px', padding: '24px', maxWidth: '500px' }}>
            <h2>{selectedBook.title}</h2>
            <p><strong>Author:</strong> {selectedBook.author}</p>
            <p><strong>ISBN:</strong> {selectedBook.isbn}</p>
            <p><strong>Genre:</strong> {selectedBook.genre}</p>
            <p><strong>Stock:</strong> {selectedBook.availableCopies || 0} / {selectedBook.totalCopies || 1}</p>
            <button onClick={closeDetails} style={{ marginTop: '16px', padding: '8px 16px' }}>Close</button>
          </div>
        </div>
      )}
    </div>
